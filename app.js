...
function refresh(){const el=$('ticker');if(!el)return;const search=$('tickerSearch');const current=[...selectedTickers];const all=!current.length||current.length===data.length;el.innerHTML='<option value="ALL">ALL SAHAM · '+data.length+'</option>'+data.map(x=>'<option value="'+esc(x.ticker)+'">'+esc(x.ticker)+(x.name?' — '+esc(x.name):'')+'</option>').join('');el.value=all?'ALL':(current[0]||'ALL');el.onchange=e=>{selectedTickers=e.target.value==='ALL'?new Set(data.map(x=>x.ticker)):new Set([e.target.value]);autoLoad()};if(search){search.oninput=()=>{const q=search.value.trim().toUpperCase();const opts=[...el.options];opts.forEach(o=>{o.hidden=!!q&&o.value!=='ALL'&&!o.textContent.toUpperCase().includes(q)});if(q&&el.value==='ALL'){const hit=opts.find(o=>o.value!=='ALL'&&!o.hidden);if(hit){el.value=hit.value;selectedTickers=new Set([hit.value])}}};search.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();const hit=[...el.options].find(o=>!o.hidden&&o.value!=='ALL');if(hit){el.value=hit.value;selectedTickers=new Set([hit.value]);autoLoad()}}}}}
