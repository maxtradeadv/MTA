...
function refresh(){const el=$('ticker');if(!el)return;const search=$('tickerSearch'),combo=search?.closest('.stockcombo');const current=[...selectedTickers];const all=!current.length||current.length===data.length;el.innerHTML='<option value="ALL">ALL SAHAM · '+data.length+'</option>'+data.map(x=>'<option value="'+esc(x.ticker)+'">'+esc(x.ticker)+(x.name?' — '+esc(x.name):'')+'</option>').join('');el.value=all?'ALL':(current[0]||'ALL');if(search){search.value=all?'':(current[0]||'');search.onfocus=()=>{combo?.classList.add('open');search.setAttribute('aria-expanded','true');filterTickerOptions()};search.onclick=()=>{combo?.classList.add('open');search.setAttribute('aria-expanded','true');filterTickerOptions()};search.oninput=()=>filterTickerOptions();search.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();const hit=[...el.options].find(o=>!o.hidden&&o.value!=='ALL');if(hit){chooseTicker(hit.value)}}else if(e.key==='Escape'){closeTickerPicker()}};el.onclick=e=>{const opt=e.target.closest('option');if(opt)chooseTicker(opt.value)};el.onchange=e=>chooseTicker(e.target.value);document.addEventListener('click',closeTickerOutside,{once:false})}}
function filterTickerOptions(){const search=$('tickerSearch'),el=$('ticker'),combo=search?.closest('.stockcombo');if(!search||!el)return;const q=search.value.trim().toUpperCase();let first=null;[...el.options].forEach(o=>{o.hidden=!!q&&o.value!=='ALL'&&!o.textContent.toUpperCase().includes(q);if(!first&&!o.hidden&&o.value!=='ALL')first=o});el.value=q?(first?.value||'ALL'):(selectedTickers.size===data.length?'ALL':([...selectedTickers][0]||'ALL'));combo?.classList.add('open');search.setAttribute('aria-expanded','true')}
function chooseTicker(t){const el=$('ticker'),search=$('tickerSearch'),combo=search?.closest('.stockcombo');if(t==='ALL'){selectedTickers=new Set(data.map(x=>x.ticker));search.value=''}else{selectedTickers=new Set([t]);search.value=t}el.value=t;[...el.options].forEach(o=>o.hidden=false);combo?.classList.remove('open');search?.setAttribute('aria-expanded','false');autoLoad()}
...
function closeTickerOutside(e){const combo=$('tickerSearch')?.closest('.stockcombo');if(combo&&!combo.contains(e.target))closeTickerPicker()}
...
function stamp(){const t=new Date().toLocaleString('id-ID',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'Asia/Jakarta'}).replace(',', '');const old=$('lastUpdate');if(old)old.textContent=t;const out=$('lastUpdateInline');if(out)out.textContent='Data Connection · diperbarui '+t+' WIB';return t}function appLog(source){const el=$('providerInfoText');if(el)el.textContent=source+' · diperbarui '+stamp()+' WIB'}function setLiveStatus(x){const old=$('liveStatus');if(old)old.textContent=x;const out=$('liveStatusInline');if(out)out.textContent='Data Connection · '+x+' · '+stamp()+' WIB'}
