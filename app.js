...
function stamp(){const t=new Date().toLocaleString('id-ID',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'Asia/Jakarta'}).replace(',', '');const old=$('lastUpdate');if(old)old.textContent=t;const out=$('lastUpdateInline');if(out)out.textContent=t;return t}function appLog(source){const el=$('providerInfoText');if(el)el.textContent=source+' · diperbarui '+stamp()+' WIB'}function setLiveStatus(x){const old=$('liveStatus');if(old)old.textContent=x;const out=$('liveStatusInline');if(out)out.textContent=x}
