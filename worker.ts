...
import { remoteCsvDay, remoteCsvRange, remoteCsvServerTimestamp } from './providers/remote-csv.ts';
...
interface Env { ASSETS: Fetcher; APP_ACCESS_CODE?: string; APP_SESSION_SECRET?: string; INDEX_ALPHA_API_KEY?: string; DATA_PROVIDER?: string; }
let runtimeEnv: Env;
const envGet=(k:keyof Env)=>runtimeEnv?.[k] ?? '';
const providerOf=(v:string|null)=>['idx','indexalpha','yahoo','remote-csv','auto'].includes(v||'')?v as Provider:(['idx','indexalpha','yahoo','remote-csv'].includes(envGet('DATA_PROVIDER')||'')?envGet('DATA_PROVIDER') as Provider:'auto');
const hasIA=()=>!!envGet('INDEX_ALPHA_API_KEY')?.trim();
...
async function sessionKey(){
  const access=envGet(ACCESS_CODE_ENV)?.trim();
  if(!access)return null;
  const secret=(envGet(SESSION_SECRET_ENV)?.trim()||access);
...
async function authGuard(req:Request){
  const configured=!!envGet(ACCESS_CODE_ENV)?.trim();
  if(!configured)return new Response('Private access is not configured. Set Cloudflare secret APP_ACCESS_CODE.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8','Cache-Control':'no-store'}});
...
export default {async fetch(req:Request,env:Env){runtimeEnv=env;(globalThis as any).__MTA_IA_KEY=env.INDEX_ALPHA_API_KEY||'';const u=new URL(req.url);try{
  ...
  if(u.pathname==='/auth/login'&&req.method==='POST'){
    ...
    const expected=envGet(ACCESS_CODE_ENV)?.trim()||'';
